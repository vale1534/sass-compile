...
_yargs["default"].option('include', {
  ...
  alias: 'r',
  requiresArg: true,
  describe: 'Replace from the start before path resolve\nSyntax: -replace pre/path;new/path'
}).option('inlines', {
  alias: 'n',
  requiresArg: true,
...
function main(argv) {
  ...
  if (argv.replace) {
    let replace = argv.replace;
    if (typeof replace === 'string') replace = [replace];
    let hasError = false;
    replace.forEach(r => {
      if (hasError) return;
      const kv = r.split(',');
      if (kv.length === 1) maps[kv[0]] = '';else if (kv.length !== 2) hasError = true;else maps[kv[0]] = kv[1];
    });
